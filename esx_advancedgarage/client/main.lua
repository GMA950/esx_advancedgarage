...
------------------------------------------------- CLIENT EVENTS QUE SPAWNEAN CON EL COMBUSTIBLE YA RECUPERADO
RegisterNetEvent('setFuel')
AddEventHandler( "setFuel", function( vehicle, plate, fuel )
	SpawnVehicle(vehicle, plate, fuel)
end)

RegisterNetEvent('setFuel2')
AddEventHandler( "setFuel2", function( vehicle, plate, fuel )
	SpawnPoundedVehicle(vehicle, plate, fuel)
end)
-------------------------------------------------
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				menu.close()
				TriggerServerEvent('esx_advancedgarage:setVehicleFuel', data.current.value.plate, data.current.value.vehicle, true)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
			...
			ESX.TriggerServerCallback('esx_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					TriggerServerEvent('esx_advancedgarage:setVehicleFuel', data.current.value.plate, data.current.value, false)
				else
...
function StoreVehicle(vehicle, vehicleProps)
	...
	ESX.Game.DeleteVehicle(vehicle)
	TriggerServerEvent('esx_advancedgarage:setVehicleState', vehicleProps.plate, true)
	TriggerServerEvent('esx_advancedgarage:saveVehicleFuel', vehicleProps.plate, fuelLevel)
	ESX.ShowNotification(_U('vehicle_in_garage'))
end
...
function SpawnVehicle(vehicle, plate, fuelLevel)
	ESX.Game.SpawnVehicle(vehicle.model, {
		...
		--
		exports["LegacyFuel"]:SetFuel(callback_vehicle, fuelLevel) -- ASIGNAMOS EL COMBUSTIBLE AL VEHICULO
		--
	end)
...
function SpawnPoundedVehicle(vehicle, plate, fuelLevel)
	ESX.Game.SpawnVehicle(vehicle.model, {
		x = this_Garage.SpawnPoint.x,
		y = this_Garage.SpawnPoint.y,
		z = this_Garage.SpawnPoint.z + 1
